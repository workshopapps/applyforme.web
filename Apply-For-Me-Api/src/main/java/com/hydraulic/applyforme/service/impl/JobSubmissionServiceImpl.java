...
public class JobSubmissionServiceImpl implements JobSubmissionService {
	...
	public JobSubmissionServiceImpl(JobSubmissionRepository repository, ApplierRepository applierRepository,
			...
			ProfessionalJpaRepository professionalJpaRepository, ApplierJpaRepository applierJpaRepository,
			ProfessionalProfileRepository professionalProfileRepository, SubmissionRepository submissionRepository) {
		this.applierRepository = applierRepository;
		this.repository = repository;
		this.repo = repo;
		this.modelMapper = modelMapper;
	...
	public ApplyForMeResponse filterJobSubmission(int pageNo, int pageSize, String sortBy, String sortDir, String q) {
		Page<Submission> submission = repository
				.findJobSubmissionBySearch(createPageable(pageNo, pageSize, sortBy, sortDir), q);
		return getSubmissionResponse(submission);
	}
	...
	@Override
	@Transactional(readOnly = true)
	public ApplyForMeResponse getEntries(int pageNo, int pageSize, String sortBy, String sortDir, String q, Date from,
			Date to) {
	...
	@Override
	@Transactional(readOnly = true)
	public ApplyForMeResponse getUserEntries(int pageNo, int pageSize, String sortBy, String sortDir, String q,
			Date from, Date to, Long userId) {
		Page<Submission> submissions = null;

		if (from != null && to != null && q != null && q.trim() != "") {
			submissions = jpaRepository.getUserEntries(from, to, q, userId,
					createPageable(pageNo, pageSize, sortBy, sortDir));
		} else if (from != null && to != null) {
			submissions = jpaRepository.getUserEntries(from, to, userId,
					createPageable(pageNo, pageSize, sortBy, sortDir));
		}
		if (q != null && q.trim() != "") {
    ...
    public Submission findOne(Long id) {
        Submission submission = repo.getOne(id);
	...
	private ApplyForMeResponse getJobSubmissionResponse(Page<Submission> submissions) {
		Collection<Submission> results = submissions.getContent().stream().map(x -> {
	...
	public Submission saveSubmission(CreateJobSubmissionDto dto) {
		...
		ProfessionalProfile existingProfessionalProfile = professionalProfileRepository
				.getOne(dto.getProfessionalProfileId());
